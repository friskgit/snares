...
**** attribute/value factory
    ...
    #+begin_src scheme :noweb yes :var attrs=attribute_list(lgth=19) val=value_list(lgth=10)
      (define valuelist
    ...
    (define valuelist
	 ...
	 (if (= i n)
	     x
	     (valuelist (+ i 1) n (append x (list (format #f "~d" i)))))))
     (valuelist 0 lgth (list))
    #+end_src

    See last line for a method for adding parameter/value pairs to the attributes list
	...
	(when (< i (length lst))
      ...
      (print-list 0 (map 
		     (lambda (x) 
		       (format #f "\\~a, ~a" (car x) (cadr x))) (append attributes (list (list "newp" 2)))))
    #+end_src
