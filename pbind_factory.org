...
***** use shift list
      Example use of [[*shift list][shift list]] with [[*value sequence][value sequence]]. The latter creates an array of numbers that are duplicated and shifted to the right
      #+begin_src scheme :noweb yes
	<<value_sequence>>       
	<<shift_list>>

	(define lst (seq-array 0 10 (list)))
...
***** value sequence
     ...
     - n: start index
     - lgth: the number of rows
     - items: the number of items in the list
     - x: the list to contain the result
     #+name: value_sequence
     ...
     value_sequence example call for the ~seq-array~ function.
     #+begin_src scheme :noweb yes
       <<value_sequence>>
       (seq-array 0 10 (list))
     #+end_src
     ...
     #+RESULTS:
     | Pseq([1.0, 0.5, ], inf) | Pseq([1.0, 0.5, ], inf) |
