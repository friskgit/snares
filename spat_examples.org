...
#+begin_src sclang :results none :tangle spatme.sc
  ~snareSimple = SynthDef(\snare, {
    ...
    snd = OGenericSnarefs.ar(imp, 0.00001, 0.001, \noiselvl.kr(0.0), \nrel.ir(0.1), frq1, frq2, \rel.ir(0.1), \trifrq.kr(200)) * EnvGen.kr(env, doneAction: Done.freeSelf);
...
Helper function for GUI. Helper function for GUI. Function to generate a line between -~range~ and ~range~ in ~time~ seconds to control a slider.
If ~dir~ is 1 the function counts from 0 and up, if it is 0 it counts from ~range~ to -~range~.
