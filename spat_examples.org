...
#+begin_src sclang :results none :noweb yes  :tangle spatme.sc
  ...
  ,*/
  ~reverbSend.source = { arg scale;
    //    var in = \in.ar(~reverbChan);
    //  in = in * \reva.kr(0);
    scale = ControlSpec(0, 1, \lin).map(\reva.kr(0) / 20);
...
#+begin_src sclang :results none
  var distanceFunc = { arg size, dir, time;
    var rout = Routine {
          size.do({ arg val, wait;
            ...
            val.postln;		
            ~hoaSignal.set(zParam, (csZ.map(val)));
            ~reverbSend.set(revSend, (rSend.map(val)));
            wait = time / size;   
            wait.wait;
          });
          "done".postln;
...
#+begin_src sclang :results none :noweb yes
  ...
  ~windTwo = ~addGuiElements.value(~guiWindow, "HOAEncoder 2", 300, \azpanb, \elpanb, \radb, \revb);
#+end_src
...
#+begin_src sclang :results none :noweb yes :tangle test.sc
  ~addGuiElements = { arg window, name = "default", offset = 0, xParam = \azpana, yParam = \elpana, zParam = \rada, revSend = \reva, rVol=\revVol;
    ...
    cSlid.action_({
          ...
          ~reverbSend.set(revSend, (rSend.map(cSlid.value)));
    ...
    .action_({ arg butt, revC, rout;
          if(butt.value == 1, {
            distanceFunc.value(200, 1, 10);	  
          }, {
            distanceFunc.value(200, 0, 10);	  	  
            butt.value.postln;
          });
    });
  };
...
The reverb is [[reverb_ambi][encoded here]] (and decoded)
#+name: reverb_ndef
#+begin_src sclang :results none
  Ndef(\reverb, {
    var src; src = \in.ar(0!2) * \amp.kr(1);
    src = JPverb.ar(
          src,
          \t60.kr(1,           0.05),
          \damp.kr(0,          0.05),
...
#+begin_src sclang :results none :noweb yes
  ...
  Ndef(\reverb).play(61);
  // Change the above to the below later
  ~reverbEncoder <<> Ndef(\reverb);
  // ~decoder.add(~reverbEncoder)
#+end_src
