...
#+begin_src sclang :results none :noweb yes :tangle test.sc
  ~addGuiElements = { arg window, name = "default", offset = 0, multi = pi, xParam = \azpana, yParam = \elpana, zParam = \rada, rev = \reva, rVol=\revVol;
    var text, dNum, eNum, fNum, rNum, sNum, aSlid, bSlid, cSlid, revVol, csX, csY, csZ, rSend, vol, rv, revScale;
    ...
    rv = ControlSpec(0.8, 0.2, \linear, 0.001, 0);
    ...
    cSlid.action_({
          ...
          ~reverbSend.set(rev, (rSend.map(cSlid.value)));
...
#+begin_src sclang :results none
  ...
  //  Ndef(\revL).play(addAction: \addToTail);
  //  Ndef(\revR).play(addAction: \addToTail);
  //  Ndef(\revL) <<>.input Ndef(\reverb);
  //  Ndef(\revR) <<>.input Ndef(\reverb);
  ~reverbEncoder <<> Ndef(\reverb);
  Ndef(\reverb).set(\t60, 0.5);
  Ndef(\reverb).set(\size, 0.5);
  Ndef(\reverb).set(\amp, 0.6);
