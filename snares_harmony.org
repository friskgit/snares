...
#+begin_src sclang :results none
  ...
  c = Synth.new(\p_snare, [\freq, [20, 40]]);
  c.map(\pulse, b);
  b.set(400);
  b.get.postln;
#+end_src
...
#+name: add_audio_bus
#+begin_src sclang :results none
  ~addAudioBusses = { arg audioBusArray, numOfBusses = 1;
    var bus, offset = 0, return;
    offset = audioBusArray.size;
    numOfBusses.do( { arg i;
          bus = Bus.audio(s, 1);
          return = audioBusArray.add(bus);
    });
    return;
  };
#+end_src
...
#+begin_src sclang :results none :noweb yes
  <<add_audio_bus>>
  ~audioBusses = ~addAudioBusses.value(~audioBusses, 1);
...
#+begin_src sclang :results none :noweb yes
  <<interpolate_busses>>
  <<pulse_modulation>>
  ~modulateSpeed.value(2, 10, 500, \change, [2,3]);
#+end_src
...
#+begin_src sclang :results none
  ~setSpatPositions.value([ [ 0, 1, 1 ], [ 1, 1, 1 ] ], 2);
