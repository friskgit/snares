...
- amp_mode: the relation between the amplitude of the notes in the array.
#+name: harmonics
...
#+begin_src sclang :results none
  ~interpolatePulse.value(~modulateSpeed.value(2, 20, 3, \multi));
#+end_src
...
#+begin_src sclang :results none
  { Poll.kr(Impulse.kr(10), In.kr(~controlBus[0].index + 2), \test) }.play(s);
#+end_src
...
#+begin_src sclang :results none
  ~setSpatialization = { arg preset, time;
    switch(preset,
          \simple, { ~setSpatPositions.value([[0, 0.0, 0.0], [5, 0.0, 0.0]]); },
