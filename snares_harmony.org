...
#+begin_src sclang :results none
  ...
  var revMap;
  ~spatNdefsAmbi = Array.new(~audioBusses.size);
  ...
  ~audioBusses.do({ arg bus, i;
    ...
    ~hoaSignal[0] = { HOAEncoder.ar(~order, In.ar(bus, 1), \azpana.kr(-3.14), \elpana.kr(0), \gaina.kr(0), plane_spherical:1) };
    ...
    if(~space == 0,
          ...
          {
            ~spatNdefs.add(
                  Ndef.new(m, { arg src, azi = 0, ele = 0, dist = 0;
                    src = In.ar(bus);
                    azi = In.kr(n);
                    ele = In.kr(n.index + 1);
                    dist = LinLin.kr( In.kr(n.index + 2).cubed, 0, 1, 0.0001, 1);
                    HOAEncoder.ar(~order, In.ar(bus, 1), azi, ele, dist, plane_spherical:1)
                  });
            );
          });  
#+end_src
*** Example with control
    ...
    #+call: connect()
    #+call: cleanup()
    Sets the environment variables (~~order~, ~~hoaNumChannels~, ~~decodeNumChannels~), then, creates the encoder and the decoder.
    Two encoders in parallel feeding to the same decoder. 
    Automation for the panning is controlled by ~~azimuthA~ and ~~azimuthB~ that are connected to the encoders.
    #+name: encode
    #+begin_src sclang :results none :noweb yes
      (
      // Settings
      ...
      ~azimuthA.source = { SinOsc.kr(0.1, 0, 0.9pi, 0); };
      ~azimuthB.source = { SinOsc.kr(2, 0, 0.5pi, 0); };
    ...
    Control the panning position C
    #+begin_src sclang :results none
      ~hoaSignal.set(\azpana, -3.14);
      ~hoaSignal.set(\elpana, -3.14);
      ~hoaSignal.set(\azpanb, -3.14);
    ...
    Control the panning position D
    #+begin_src sclang :results none
