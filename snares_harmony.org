...
#+begin_src sclang :results none
  ...
  ~synths[0].get(\freq, { arg value; ("freq2 is now:" + value + "Hz").postln; });
#+end_src
...
#+begin_src sclang :results none
~updatePanningAmbiSingle.value(0, 0, -1.5, 1, 8);
#+end_src
...
Fade out.
#+begin_src sclang :results none
  ~setSpatPositions.value([ [ 0, 1, 90 ], [ 1, 1, 90 ] ], 4);
#+end_src
