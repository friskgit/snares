...
#+begin_src sclang :results none :noweb yes
  <<interpolate_busses>>
  <<pulse_modulation>>
  ~modulateSpeed.value(1, 6, 1, \ratio, [2,3]);
  ~setSpatPositions.value([[0, 1, 1], [0, 1, 1]], 6);
#+end_src

#+begin_src sclang :results none
  ~synths[0].set(\pulse, 331);
  ~synths[1].set(\pulse, 331);
  ~setSpatPositions.value([[7, 1, 10], [1, 1, 10]], 5);
#+end_src
...
Bring forward in one angle
#+begin_src sclang :results none
  ~setSpatPositions.value([ [0, 1, 1] ], 3);
  //~controlBus[1].scope;
#+end_src
