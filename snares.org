...
*** snare 3
    ...
    #+begin_src sclang :results none
      ...
      SynthDef(\isnare, { | inBus1=0, inBus2=1, inBus3=2, inBus4=3, outBus=0, freq=0, cBus1=1, gain=0.5, osc1=330, osc2=180, tri=111, noise=0.1, position=0, disperse=0, dur=1 |
	  var snd;
	  var env;
	  ...
	  snd = IDispersedSnare.ar(Impulse.ar(In.kr(inBus1)), position, disperse, osc_1_freq: In.kr(inBus2), osc_2_freq: In.kr(inBus2) - 50, triangle_freq: In.kr(inBus3), noise_lvl: In.kr(inBus4)) * gain * env;
	  Out.ar(outBus, snd);
      }).add;

      SynthDef(\control_synth_1, {
	  Out.kr(\bus.ir,
	      SinOsc.kr(
    ...
    #+begin_src sclang :results none
      (
      ~osc_control_1 = Synth.new(\control_synth_1, [
      ...
      ~rising_line_1 = Synth.after(~osc_control_1, \control_synth_2, [
	  \bus, ~line_control_1_bus.index,
	  \mult, 500,
	  \add, 50,
	  \dur, 2]);
      ~falling_line_1 = Synth.after(~osc_control_1, \control_synth_2, [
	  \bus, ~line_control_2_bus.index,
	  \start, 200,
	  \end, 40,
	  \dur, 2]);
      ~isnare_synth = Synth.after(~rising_line_1, \isnare, [
	  \inBus1, ~osc_control_1_bus.index,
    ...
    Play the synths and the modulators with Pbinds
    #+begin_src sclang :results none
       ~isnare_synth = Synth.after(~rising_line_1, \isnare, [
	  \inBus1, ~osc_control_1_bus.index,
	  \inBus2, ~line_control_1_bus.index,
    ...
    #+begin_src sclang :results none
      {Poll.kr(Impulse.kr(10), In.kr(~controlBus_2.index))}.play;
    #+end_src
