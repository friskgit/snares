...
trif = syngroup(hslider("[2]triangle freq", 111, 10, 5000, 0.1));
osc1f = syngroup(hslider("[0]osc 1 freq", 360, 10, 5000, 0.1));
osc2f = syngroup(hslider("[1]osc 2 freq", 180, 10, 5000, 0.1));
...
with {
  ...
  sustain = envgroup(hslider("[3]noise sustain", 0.5, 0, 1, 0.0001) : si.smooth(0.1));
  rel = envgroup(hslider("[4] noise rel", 0.1, 0.0000001, 0.5, 0.0001) : si.smooth(0.2));
};

// Noise
nse_vol = nsegroup(hslider("noise vol", 0.1, 0, 1.0, 0.0000001) : si.smooth(0.1));
...
f2smp(freq) = (mSR, freq:/) ;
phasor(smp) = syngroup(hslider("modulation freq", 1, 0, 10, 0.0001)) : +~_ : _,smp : fmod : _,smp : / ;
...
process(x) = trimod : par(i, 2, _ * env(x)), oscs(x) :> _,_ , nse(x)  :> _; 
